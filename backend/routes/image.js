...
router.delete('/image/:imageId', async function (req, res, next) {
    ...
    try {
        ...
        const [
            ...
            "SELECT `file_path` FROM `product_image` WHERE `image_id` = ?",
            [req.params.imageId]
        );
        ...
        const [rows1, fields1] = await conn.query(
            'DELETE FROM `product_image` WHERE `image_id`=?', [req.params.imageId]
        )
...
router.put('/image/setmain/:blogId/:imageId', async function (req, res, next) {
    ...
    try {
        ...
        await conn.query(
            'UPDATE `product_image` SET `main`=0 WHERE `product_id`=?;', [req.params.blogId]
            // 'UPDATE `images` SET `main`="0" WHERE `blog_id`=?'[req.params.blogId]
        )
        await conn.query(
            'UPDATE `product_image` SET `main`=1 WHERE `image_id`=?', [req.params.imageId]
